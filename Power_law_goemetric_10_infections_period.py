...
print("sdfsdfplt.style.use('ggplot')")
plt.style.use('ggplot')

print("Task 3.5: Infection Period Analysis 1 infection")
std_values_low = np.linspace(4.5,20,40) 
std_values_low_mid = np.linspace(21,60,40)
std_mid = np.linspace(65,100,35)
std_high = np.linspace(105,330,30)
std_high_high = np.linspace(350,10000,250) 
...
def get_network_infection_data(base_network_orig, std_dev_values, 
    ...
    for std in std_dev_values:
        ...
        if "Geometric" in network_label_for_condition and std <= 7.5:
        ...
        if ("Power Law" in network_label_for_condition) or ("Geometric" in network_label_for_condition and std > 7.5):
        ...
        if "Geometric" in network_label_for_condition and std>10: # Negative assortativity
...
if __name__ == "__main__":
    ...
    # To ensure the Watts-Strogatz legend entry appears only once
    # ws_legend_added = False # Not needed if WS is commented out
    ...
    for config in network_configs:
        ...
        if "Watts-Strogatz" in config["name"] and "actual_p_rewire" in config:
            print(f"\nProcessing base network type: Watts-Strogatz (p={config['actual_p_rewire']:.3f})")
        else:
            print(f"\nProcessing base network type: {config['name']}")
        
        base_network = config["network_creator"]() 
        
        # initial_infected_nodes is now sampled within the helper functions or the else block below
        # initial_infected_nodes = []
        # if base_network.num_nodes > 0 :
        #     available_nodes_to_sample = list(base_network.adj.keys())
        #     if not available_nodes_to_sample and base_network.num_nodes > 0: 
        #         available_nodes_to_sample = list(range(base_network.num_nodes))

        #     if available_nodes_to_sample:
        #         k_to_infect = min(num_initial_to_infect_config, len(available_nodes_to_sample))
        #         if k_to_infect > 0:
        #             initial_infected_nodes = random.sample(available_nodes_to_sample, k=k_to_infect)
        #         # ... (rest of the original sampling logic)
        ...
        if config.get("apply_assortativity_modification", True): 
            ...
            avg_time_list = []
            # Assuming base_network has nodes for sampling
            ...
            for _ in range(num_avg_runs_infection_length):
                ...
                time = length_of_infection_period(base_network, current_initial_infected_base, fixed_lambda, num_weeks_simulation_config)
                avg_time_list.append(time)
            time_data.append(np.mean(avg_time_list))
        
        current_label = config["name"]
        if "Watts-Strogatz" in config["name"]:
            if not ws_legend_added:
                plot_label = current_label
                ws_legend_added = True
            else:
                plot_label = None # No label for subsequent WS points to avoid duplicate legend entries
        else:
            plot_label = current_label


        if r_data: 
            axs[0].scatter(r_data, time_data, color=config["base_color"], marker=config["marker"], label=plot_label, alpha=0.7)
        if c_data:
            axs[1].scatter(c_data, time_data, color=config["base_color"], marker=config["marker"], label=plot_label, alpha=0.7)

    axs[0].set_xlabel('Assortativity Coefficient (r)')
    axs[0].set_ylabel('Average Length of Infection Period (Weeks)')
    axs[0].set_title(f'Infection Length vs. Assortativity (λ={fixed_lambda:.2f})')
    axs[0].legend(fontsize='small')
    axs[0].grid(True)

    axs[1].set_xlabel('Clustering Coefficient (C_g)')
    axs[1].set_ylabel('Average Length of Infection Period (Weeks)') 
    axs[1].set_title(f'Infection Length vs. Clustering (λ={fixed_lambda:.2f})')
    axs[1].legend(fontsize='small')
    axs[1].grid(True)
    
    # Add a more descriptive super title
    fig.suptitle(f'Infection Period Analysis with Assortativity Modifications (λ={fixed_lambda:.2f}, {num_initial_to_infect_config} initial infections)', fontsize=16)
    plt.tight_layout(rect=[0, 0.03, 1, 0.95]) # Adjust rect to make space for suptitle
    
    filename = f"Infection_period_analysis_lambda_{fixed_lambda:.2f}_{num_initial_to_infect_config}infections_modified_assort.png" # Made filename more descriptive
