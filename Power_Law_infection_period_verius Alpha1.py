...
if __name__ == "__main__":
    ...
    network_configs = [ # Renamed to avoid confusion inside the lambda loop
        {
            "name": f"Power Law (α={p1}, N=10000)",
            "network_creator": lambda alpha_val=p1: Network.from_power_law(alpha=alpha_val, num_nodes=10000, min_deg=1, max_deg=100),
            "marker": 'x', # New marker for Power Law
            "base_color": 'red', # New color for Power Law
            "apply_assortativity_modification": True
        },
        {
            "name": f"Power Law (α={p2}, N=10000)",
            "network_creator": lambda alpha_val=p2: Network.from_power_law(alpha=alpha_val, num_nodes=10000, min_deg=1, max_deg=100),
            "marker": 'x', # New marker for Power Law
            "base_color": 'green', # New color for Power Law
            "apply_assortativity_modification": True
        },
        {
            "name": f"Power Law (α={p3}, N=10000)",
            "network_creator": lambda alpha_val=p3: Network.from_power_law(alpha=alpha_val, num_nodes=10000, min_deg=1, max_deg=100),
            "marker": 'x', # New marker for Power Law
            "base_color": 'blue', # New color for Power Law
            "apply_assortativity_modification": True
        }
    ]
    ...
    lam_values_main = [1]  # Reduced for faster testing, adjust as needed
    ...
    for lam_val in lam_values_main:
        ...
        save_filename = f'NEWNpeak_infection_metrics_lam_123{lam_val:.2f}1infection.svg'
